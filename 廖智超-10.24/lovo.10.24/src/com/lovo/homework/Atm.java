...
public class Atm {
	...
	private UserBean[] users = new UserBean[5];// 用户
	...
	public Atm() {
		...
		for(int i = 0;i<users.length;i++){
			users[i]=new UserBean("123"+i,"mima"+i,1000*i);
		}
	}
	...
	private boolean login() {
		...
		for (int i = 0; i < 3; i++) {
			...
			//改动处，将users中的user赋给this.user
			for(UserBean user:users){
				if(inputCardNum.equals(user.getCardNum()) && inputPwd.equals(user.getPassword())){
			...
			if(!flag) System.out.println("卡号或密码有误");			 
		}
